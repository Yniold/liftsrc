...
for i=0:39
  ...
  if (x~=0) %no problem with temperature card
    ...
    elseif strcmp('3E76',Tempid(i+1,:)) colTempAxisPlate=colTemp0WP+3*i;
    ...
    elseif strcmp('57DA',Tempid(i+1,:)) colTempCalPlate=colTemp0WP+3*i;
    elseif strcmp('87F9',Tempid(i+1,:)) colTempWZ1in=colTemp0WP+3*i;
    elseif strcmp('5AC5',Tempid(i+1,:)) colTempWZ2in=colTemp0WP+3*i;
    elseif strcmp('A2EF',Tempid(i+1,:)) colTempSensCard=colTemp0WP+3*i; %A2E5?
    ...
    elseif strcmp('A2E5',Tempid(i+1,:)) colTempArmTempCard=colTemp0WP+3*i;
    elseif strcmp('39AD',Tempid(i+1,:)) colTempDetFunnel=colTemp0WP+3*i;
    elseif strcmp('4B68',Tempid(i+1,:)) colTempPrallhz=colTemp0WP+3*i;
...
if exist('colTempDetFunnel')==0 colTempDetFunnel=NaN; end
...
'DiodeUV',671,'0.00743605*(x-10001)','mW'; %calibrated by Korbinian, 10th June 2010
'DiodeUVSumDat',672,'0.00743605*(x-10001)','mW';
'DiodeUVSumSqr',673,'0.00743605*(x-10001)','mW';
...
%Gabriel, HOOVER1 TDet -50-0 �C
%'TDet',930,'1./(3.3538646E-3+2.565409E-4.*log(680./3000.*(5.*1750./(x-10000)-1))+1.9243889E-6.*(log(680./3000.*(5.*1750./(x-10000)-1))).^2+1.0969244E-7.*(log(680./3000.*(5.*1750./(x-10000)-1))).^3)-273.15','C';
%TDet -50-0 �C
%'TDet',930,'1./(3.3538646E-3+2.565409E-4.*log(220000./3000./(4.094.*1550./(x-9999)-1))+1.9243889E-6.*(log(220000./3000./(4.094.*1550./(x-9999)-1))).^2+1.0969244E-7.*(log(220000./3000./(4.094.*1550./(x-9999)-1))).^3)-273.15','C';
%TDet 0-50 �C:
...
'GPSMsecondsUTC',867,'x','s';
'GPSMLongitude',868,'x/60-180','deg';
...
'GPSMGroundSpeed',876,'x/100','m/s';
'GPSMHeading',877,'x/10','deg';
'GPSSsecondsUTC',1111,'x','s';
'GPSSLongitude',1112,'x/60-180','deg';
...
'GPSSGroundSpeed',1120,'x/100','m/s';
'GPSSHeading',1121,'x/10','deg';
