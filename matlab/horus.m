...
if ( double(statusData(lastrow,5))<10 & ... % in the first 10 seconds of a minute
    ...
    if (mod(double(statusData(lastrow,4)),4)<2) % in the first 2 minutes of a 4 minute interval
        ...
        if (~bitget(statusData(lastrow,col.Valve1armAxis),14)) % Valve still closed
            ...
            system(['/lift/bin/eCmd @armAxis w 0xa442 ', num2str(uint16(255*190/200))]); % set scale 50 sccm Propene flow of 200 sccm MFC to 255
...
data.TDyelaserset=38; % define temperature Dye Laser heating is set to
if isfield(data,'hDyelaser')
    if ishandle(data.hDyelaser)
...
if isfield(data,'hFlyDetection')
    if ishandle(data.hFlyDetection) 
        ...
        if P20(lastrow)<1 | P20(lastrow)>10 | DiodeWZ1in(lastrow)<2 | DiodeWZ2in(lastrow)<0.4 ...
                | MFCFlow(lastrow)<4 | MFCFlow(lastrow)>9 ...
...
%            | DiodeWZ1out(lastrow)<0.6*DiodeWZ1in | DiodeWZ2out(lastrow)<0.6*DiodeWZ2in ...
            set(handles.FlyDetection,'BackgroundColor','r');
        else
            set(handles.FlyDetection,'BackgroundColor','c');
        end
