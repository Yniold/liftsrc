...
switch char(xpar)
    ...
    case 'MFCC3F6Flow'
        if get(handles.toggleX,'Value')==0
            xdata=statusData(:,col.MFCC3F6Flow);
        else
            x=double(statusData(:,col.MFCC3F6Flow)); eval(['xdata=',fcts2val.MFCC3F6Flow,';']);
        end
    case 'MFCC3F6SetFlow'
        if get(handles.toggleX,'Value')==0
            xdata=statusData(:,col.MFCC3F6SetFlow);
        else
    ...
    case 'MFCShowerFlow'
        if get(handles.toggleX,'Value')==0
            ...
            x=double(statusData(:,col.MFCShowerFlow)); eval(['xdata=',fcts2val.MFCShowerFlow,';']);
        end
    case 'MFCShowerSetFlow'
        if get(handles.toggleX,'Value')==0
            xdata=statusData(:,col.MFCShowerSetFlow);
        else
            x=double(statusData(:,col.MFCShowerSetFlow)); eval(['xdata=',fcts2val.MFCShowerSetFlow,';']);
        end
    case 'PitotAbs'
        if get(handles.toggleX,'Value')==0
    ...
    case 'TempDetFunnel'
        if ~isnan(col.TempDetFunnel)
            if get(handles.toggleX,'Value')==0
                xdata=statusData(:,col.TempDetFunnel);
            else
                x=double(statusData(:,col.TempDetFunnel)); eval(['xdata=',fcts2val.TempDetFunnel,';']);
            end
        else
            xdata=statustime;
            xdata(:)=NaN;
    ...
    case 'TempDetFunnelid'
        if ~isnan(col.TempDetFunnelid)
            if get(handles.toggleX,'Value')==0
                xdata=statusData(:,col.TempDetFunnelid);
            else
                x=uint16(statusData(:,col.TempDetFunnelid)); eval(['xdata=',fcts2val.TempDetFunnelid,';']);
            end
        else
            xdata=statustime;
            xdata(:)=NaN;
...
switch char(ypar)
    ...
    case 'MFCC3F6Flow'
        if get(handles.toggleY,'Value')==0
            ydata=statusData(:,col.MFCC3F6Flow);
        else
            x=double(statusData(:,col.MFCC3F6Flow)); eval(['ydata=',fcts2val.MFCC3F6Flow,';']);
        end
    case 'MFCC3F6SetFlow'
        if get(handles.toggleY,'Value')==0
            ydata=statusData(:,col.MFCC3F6SetFlow);
        else
            x=double(statusData(:,col.MFCC3F6SetFlow)); eval(['ydata=',fcts2val.MFCC3F6SetFlow,';']);
        end
    case 'MFCPropFlow'
        if get(handles.toggleY,'Value')==0
            ydata=statusData(:,col.MFCPropFlow);
        else
            x=double(statusData(:,col.MFCPropFlow)); eval(['ydata=',fcts2val.MFCPropFlow,';']);
        end
    case 'MFCPropSetFlow'
        if get(handles.toggleY,'Value')==0
            ydata=statusData(:,col.MFCPropSetFlow);
        else
    ...
    case 'MFCShowerFlow'
        if get(handles.toggleY,'Value')==0
            ...
            x=double(statusData(:,col.MFCShowerFlow)); eval(['ydata=',fcts2val.MFCShowerFlow,';']);
        end
    case 'MFCShowerSetFlow'
        if get(handles.toggleY,'Value')==0
            ydata=statusData(:,col.MFCShowerSetFlow);
        else
            x=double(statusData(:,col.MFCShowerSetFlow)); eval(['ydata=',fcts2val.MFCShowerSetFlow,';']);
        end
    case 'PitotAbs'
        if get(handles.toggleY,'Value')==0
    ...
    case 'TempDetFunnel'
        if ~isnan(col.TempDetFunnel)
            if get(handles.toggleY,'Value')==0
                ydata=statusData(:,col.TempDetFunnel);
            else
                x=double(statusData(:,col.TempDetFunnel)); eval(['ydata=',fcts2val.TempDetFunnel,';']);
            end
        else
            ydata=statustime;
            ydata(:)=NaN;
    ...
    case 'TempDetFunnelid'
        if ~isnan(col.TempDetFunnelid)
            if get(handles.toggleY,'Value')==0
                ydata=statusData(:,col.TempDetFunnelid);
            else
                x=uint16(statusData(:,col.TempDetFunnelid)); eval(['ydata=',fcts2val.TempDetFunnelid,';']);
            end
        else
            ydata=statustime;
            ydata(:)=NaN;
...
vars={'Time';'DiodeGr';'DiodeUV';'DiodeEtalon';'DiodeWZ1in';'DiodeWZ1out';'DiodeWZ2in';'DiodeWZ2out';...
    ...
    'TempPrallpl';'TempAxis';'TempScrollMotor';'TempScrollContr';'TempArmTempCard';'TempDetFunnel';'TempPrallhz';'TempFiberTube';...
    ...
    'TempPrallplid';'TempAxisid';'TempScrollMotorid';'TempScrollContrid';'TempArmTempCardid';'TempDetFunnelid';'TempPrallhzid';'TempFiberTubeid';...
