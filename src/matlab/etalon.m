...
onlinepos=uint8(str2double(get(hObject,'String')));
if isnan(onlinepos)
...
scanstartpos=uint8(str2double(get(hObject,'String')));
...
scanstoppos=uint8(str2double(get(hObject,'String')));
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
scanstartpos=uint8(str2double(get(handles.scan_start_pos,'String')));
scanstoppos=uint8(str2double(get(handles.scan_stop_pos,'String')));
scanstep=uint8(str2double(get(handles.scan_step,'String')));
if {isnan(scanstartpos)| isnan(scanstoppos) | isnan(scanstep)}
    error('invalid values');
else
    eval(['./bin/eCmd s etalonscanstart ',num2str(scanstartpos,0)]);
    eval(['./bin/eCmd s etalonscanstop ',num2str(scanstoppos,0)]);
    eval(['./bin/eCmd s etalonscanstep ',num2str(scanstep,0)]);
    eval('./bin/eCmd s etalonscan');
end
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
onlinepos=uint8(str2double(get(handles.online_pos,'String')));
if isnan(onlinepos)
    error('invalid values');
else
    eval(['./bin/eCmd w 0xa510 ',num2str(onlinepos,0)]);
end
...
if isnan(onlinepos)
    ...
    eval(['./bin/eCmd w 0xa510 ',num2str(offlinepos,0)]);
end
...
if isnan(onlinepos)
    ...
    eval(['./bin/eCmd s etalononline ',num2str(onlinepos,0)]);
    eval(['./bin/eCmd s etalonofflineleft 1000']);
    eval(['./bin/eCmd s etalonofflineright 1000']);
    eval('./bin/eCmd s etalondither 8');
    eval('./bin/eCmd s etalontoggle');
end
