	...
	for (Card=0; Card<MAX_MFC_CARD_LIFT; Card++)
	  {
	     int iChannel = 0;
	     for(iChannel; iChannel < 4; iChannel++)
	     printf("Ch:%01d(S%05d/F%5d) ", iChannel, ptrElekStatus->MFCCardMaster[Card].MFCChannelData[iChannel].SetFlow,
		    ptrElekStatus->MFCCardMaster[Card].MFCChannelData[iChannel].Flow);
	  }
	// for Card
	//
	printf("MFC(S):");
	...
	switch(ucChar)
	  {
	   ...
	   case 'F':					// MFC Data
	     uiGroupFlags ^= GROUP_MFCDATA;
	     if (uiGroupFlags & GROUP_MFCDATA)
	       printf("\n\rDisplay of MFC DATA is now on.\n\r");
	     else
	       printf("\n\rDisplay of MFC DATA is now off.\n\r");
	     break;
...
void ShowHelp(void)
{
   ...
   printf("[F] MFC DATA    \t[S] SHOW ALL        \t[R] RESET ALL\n\r");
